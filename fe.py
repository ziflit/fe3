...
def get_data_tsv(loc_dataset, dimensiones):
        ...
        if e > 0:
            ...
            if len(r) == 3: #train set
                for word in r[2].split(): 
                    posicion = hash(word)%dimensiones
                    if posicion not in features:
                        features.append(posicion)
                label = int(r[1])
            else: #test set
                features = [(hash(f)%dimensiones) for f in r[1].split()]
                label = 1
...
def cargarMatriz(archivo, dimensiones):
    m = []
    for i, (label, id, features) in enumerate( get_data_tsv(archivo, N) ):
...
if __name__ == "__main__":
    ...
    N = 550
    K = 5
    entrenamiento = cargarMatriz("labeledTrainData.tsv",N) # matriz con reviews de entrenamiento.
    ...
    prueba = cargarMatriz("testData.tsv", N) 